...
**valuelists** => _..._)
    ...
    [collectd.conf(5)](./collectd.conf.md) for details). If the **plugins** argument has been omitted,
...
- **plugin\_get\_interval** ()
    ...
    `LoadPlugin perl` block or the global interval (see [collectd.conf(5)](./collectd.conf.md) for
    details).
...
- **fc\_register** (_type_, _name_, _proc_)
    ...
    block (see [collectd.conf(5)](./collectd.conf.md) for details).
    ...
    **match** or **invoke** callback. **match** is called whenever processing an
    ...
    [collectd.conf(5)](./collectd.conf.md) for details). Just like any other callbacks, filter chain
...
- **$hostname\_g**
    ...
    **FQDNLookup** configuration options (see [collectd.conf(5)](./collectd.conf.md) for details).
...
- **$interval\_g**
    ...
    **Note:** This variable should no longer be used in favor of
    ...
    [collectd.conf(5)](./collectd.conf.md) for details).
